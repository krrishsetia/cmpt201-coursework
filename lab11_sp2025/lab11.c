...
int main() {
  ...
  if (!pubkey_file) {
    handle_error(" failed to open public key file");
  }
...
#define MAX_FILE_SIZE 512
  ...
  if (!mdctx)
    handle_error("error in creating mdctx");

  int result = -1;

  if (EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey) != 1)
    handle_error("error in EVP_DigestVerifyInit");

  if (EVP_DigestVerifyUpdate(mdctx, message, message_len) != 1)
    handle_error("error in EVP_DigestVerifyUpdate");
